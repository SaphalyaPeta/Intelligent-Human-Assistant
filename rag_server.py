...
async def correct_command(query: str) -> str:
    ...
    result = subprocess.run([
        'ollama', 'run', 'gemma-3-vcc', 
        query
    ], capture_output=True, text=True, timeout=30)
    
    return result.stdout.strip() if result.returncode == 0 else f"Error: {result.stderr}"
#endregion Voice Command Tools

'''

This is with some fixes which sometimes can be used, when the model glitches.
...
async def correct_command(query: str) -> str:
    ...
    # Generate the translated command using the fine-tuned model
    result = subprocess.run([
        'ollama', 'run', 'gemma-3-vcc', 
        query
    ], capture_output=True, text=True, timeout=30)
    ...
    if result.returncode == 0:
        ...
        # ADDED: Quick post-processing for common mistakes
        command = apply_quick_fixes(command, query)
        return command
    else:
        return f"Error: {result.stderr}"

def apply_quick_fixes(command: str, original_query: str) -> str:
    """Apply quick fixes to common model errors."""
    
    # Convert to uppercase for consistent matching
    command_upper = command.upper()
    query_lower = original_query.lower()
    
    # Dictation/Microphone fixes
    if any(word in query_lower for word in ['dictation', 'microphone', 'voice typing', 'speech to text']):
        if 'stop' in query_lower or 'end' in query_lower or 'close' in query_lower:
            return "STOP DICTATION"
        else:
            return "START DICTATION"
    
    # Calculator spelling fix
    if 'CALCULAR' in command_upper:
        return command.replace('CALCULAR', 'calculator').replace('CALCULARY', 'calculator')
    
    # Messages app fix
    if any(word in command_upper for word in ['PLAY', 'SOCIAL', 'MESSAGE']):
        if 'message' in query_lower:
            return command.replace('PLAY', 'messages').replace('SOCIAL', 'messages').replace('MESSAGE', 'messages')
    
    # Show/hide numbers fix
    if 'NUMBERS' == command_upper and 'show' in query_lower:
        return "SHOW NUMBERS"
    if 'NUMBERS' == command_upper and 'hide' in query_lower:
        return "HIDE NUMBERS"
    
    # First item fix
    if 'THE FIRST ITEM' in command_upper or 'FIRST ITEM' in command_upper:
        return command.replace('THE FIRST ITEM', '1').replace('FIRST ITEM', '1')
...
- Available Tools: add, subtract, multiply, divide, scrape_wikipedia, query_knowledge, clear_knowledge_base, correct_command, get_server_status
